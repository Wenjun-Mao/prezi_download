...
def test_youtube_extractor():
    ...
    try:
        ...
        # Test each URL and count valid YouTube URLs (not just newly added ones)
        valid_youtube_urls = 0
        for url in test_urls:
            # Check if URL would be normalized to a valid YouTube URL
            normalized = extractor._normalize_youtube_url(url)
            if normalized:
                valid_youtube_urls += 1
            # Also try to extract (first one will be added, others will be duplicates)
            extractor.extract_youtube_link(url)
        
        extracted_links = extractor.get_extracted_links()
        
        if valid_youtube_urls == 3 and len(extracted_links) == 1:  # 3 valid URLs, 1 unique link
            print("✅ YouTube extractor working correctly")
            print(f"   Found {valid_youtube_urls} valid YouTube URLs, extracted {len(extracted_links)} unique link(s)")
            ...
            print(f"   Got {valid_youtube_urls} valid URLs and {len(extracted_links)} unique links")
            return False
